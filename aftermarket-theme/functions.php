...
add_action('woocommerce_checkout_payment_exception', function($order, $e) {
    if (is_object($e) && method_exists($e, 'getMessage')) {
        update_option('am_last_checkout_error', $e->getMessage());
        $log = "[" . date('Y-m-d H:i:s') . "] CHECKOUT PAYMENT EXCEPTION: " . $e->getMessage() . "\n";
        $wp_content_dir = dirname(dirname(__DIR__));
        file_put_contents($wp_content_dir . '/error_log_aftermarket.txt', $log, FILE_APPEND);
    }
}, 10, 2);
